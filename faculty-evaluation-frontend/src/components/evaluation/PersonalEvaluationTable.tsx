...
// --- Info + Score Definition Section ---
// Đã cập nhật để thêm nút Resume vào dòng Tên
function InfoAndScoreSection({
  data,
  onUploadResume,
  role,
}: {
        ...
        <table className="w-full h-full text-xs border border-primary-light/30">
          <tbody>
            <tr className="border-b border-primary-light/30">
              <td className="px-4 py-3 bg-background-subtle font-semibold text-text-primary w-[40%]">
                評価対象者 氏名
              </td>
              <td className="px-4 py-3 text-text-primary">
                {/* Flex container để chứa Tên và Nút Upload */}
                <div className="flex items-center justify-between w-full">
                  <span className="font-medium text-sm">
                    {data.facultyInfo.name}
                  </span>

                  {/* Nút Upload Resume - Chỉ hiện thị hoặc enable cho Faculty */}
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={onUploadResume}
                    disabled={role !== "faculty"}
                    className="h-7 px-2 text-xs gap-1 bg-white border-primary/30 hover:bg-primary-lightest"
                    title="履歴書を登録する"
                  >
...
export function PersonalEvaluationTable({
  ...
  const [evaluations, setEvaluations] = useState<PersonalGoalItem[]>(
    JSON.parse(JSON.stringify(data.goals)),
  );
  ...
  // Ref cho Resume Input
  const resumeInputRef = useRef<HTMLInputElement>(null);

  // --- Handlers ---
  const handleGoalChange = (
    idx: number,
    field: keyof PersonalGoalItem,
    value: string,
  ) => {
    setGoals((prev) => {
      const newGoals = [...prev];
  ...
  const importGoalsCSV = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      ...
      if (parsed.length > 1) {
        const newGoals = parsed.slice(1).map(
          (row) =>
            ({
              id: Math.random().toString(36).substr(2, 9),
              category1: row[0] || "",
  ...
  const handleResumeFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      // Reset input value để cho phép upload lại file cùng tên nếu cần
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Info And Score Section (Truyền handler upload) */}
      <InfoAndScoreSection
        data={data}
      ...
      >
        <StickyTableToolbar
          onAddRow={addGoalRows}
          onDownloadSample={downloadGoalsCSV}
          onImportCSV={() => goalFileInputRef.current?.click()}
        />
          ...
          <table className="w-full text-sm">
            <thead className="sticky top-[57px] z-30 bg-gradient-to-r from-primary-dark to-primary text-white shadow-md">
              <tr>
                ...
                <th className="px-3 py-3 text-left font-semibold w-[200px] border-r border-white/50">
                  個人目標値（達成基準）
                </th>
                <th className="px-3 py-3 text-left font-semibold w-[200px] border-r border-white/50">
                  個人目標進捗度（期中）
                ...
                <th className="px-3 py-3 text-center font-semibold w-[50px]">
                  削除
                </th>
              </tr>
            </thead>
              ...
              {goals.map((goal, idx) => (
                ...
                >
                  ...
                  <td className="px-2 py-2 border-r border-primary-light/30">
                    <textarea
                      rows={2}
                      className="w-full p-1 border border-primary-light/30 rounded text-sm resize-y"
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {evaluations.map((goal, idx) => (
                <tr
                  key={goal.id}
                  className="border-b border-primary-light/30 hover:bg-background-subtle"
                >
                  <td className="px-3 py-3 text-center border-r border-primary-light/30">
                    <input
                      ...
                      onChange={(e) =>
                        handleEvalChange(idx, "individualScore", e.target.value)
                      }
                  ...
                  <td className="px-3 py-3 text-text-muted text-xs border-r border-primary-light/30">
                    {goal.aiSummary}
                  </td>
                </tr>
              ))}
